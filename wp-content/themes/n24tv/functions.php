...
function n24tv_enqueue_scripts(){
    ...
    // 2022-02-16: blrf: added masonry
    wp_enqueue_script('masonry', $template_uri . '/js/masonry.pkgd.min.js', ['jqeury'], null, true);
...
function n24tv_render_masonry($category_id = 0){
    ...
    $posts = N24TV::get_featured_posts($category_id, 6);
    ...
    foreach($posts as $_post){
        ...
        $o[] = '<a href="' . $_post['permalink'] . '">';
        // $o[] = $_post[$arrSizes[$post_no]]; // TODO: ziga added str_replace
        $o[] = str_replace('http://', 'https://', $_post[$arrSizes[$post_no]]); // TODO: ziga added str_replace
        $o[] = '</a>';
        $o[] = '</div>';
...
function n24tv_render_article(
    ...
    $o[] = '<a title="' . $_title . '" href="' . $permalink . '">' . str_replace('http://', 'https://', $thumbnail) . '</a>'; // ziga add str_replace
...
function n24tv_render_submenu_search(){
    ...
    $o[] = '<input type="search" id="searchInput" class="form-control" placeholder="Iskanje..." value="' . get_search_query() . '" name="s" />';
...
function n24tv_render_front_panel($config, $title = 'Panel'){
    ...
    if (strcmp($title, 'Lokalno') == 0) {
    $o[] = "<div class=\"panel panel-n24tv n24tv-front-articles\">" . do_shortcode('[quads id=3]') . '</div>' . 
           '<div class="panel panel-n24tv n24tv-front-articles">' . 
           '<div class="panel-heading">' . 
           '<a href="' . $ahref . '"><h3 class="panel-title"><span>' . $title . '</span></h3></a>' .
           '</div>' . 
           '<div class="panel-body">';
    } else if (strcmp($title, 'Kolumna') == 0) {
    $o[] = "<div class=\"panel panel-n24tv n24tv-front-articles\">" . do_shortcode('[quads id=3]') . '</div>' . 
           '<div class="panel panel-n24tv n24tv-front-articles">' . 
           '<div class="panel-heading">' . 
           '<a href="' . $ahref . '"><h3 class="panel-title"><span>' . $title . '</span></h3></a>' .
