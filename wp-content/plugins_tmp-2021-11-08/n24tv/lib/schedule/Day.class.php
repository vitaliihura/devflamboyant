...
class N24TV_Schedule_Day {
    ...
    static public function getLatest(){
        ...
        $SQL = "SELECT DISTINCT `date` FROM `" . N24TV_SCHEDULE_TABLE . "` WHERE `date` >= '" . $DT->format('Y-m-d') . "' ORDER BY `date` ASC";
        $r = $db->query($SQL);
        $ret = [];
    ...
    static public function count(DateTime $DT){
        $db = N24TV_Schedule_DB::getInstance();
        $SQL = "SELECT COUNT(`start`) AS ECount FROM `" . N24TV_SCHEDULE_TABLE . "` WHERE `date` = '" . $DT->format('Y-m-d') . "'";
        $r = $db->query($SQL);
        $rr = $r->fetch_assoc();
    ...
    public function copy(N24TV_Schedule_Day $To){
        $db = N24TV_Schedule_DB::getInstance();
        $SQL = "DELETE FROM `" . N24TV_SCHEDULE_TABLE . "` WHERE `date` = '" . $To->getDate() . "'";
        $db->query($SQL);
